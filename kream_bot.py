...
async def collect_shipments(page, max_pages=10):
    """판매자센터 발송완료 내역 수집 — partner-api 인터셉트 방식 (2026-04 KREAM DOM 전면 변경 대응).

    이전 방식(`/business/shipments` table DOM 스크래핑)이 다음 이유로 불가능:
    - URL 변경: `/business/shipments` → `/business/asks/shipment`
    - DOM 구조 전면 교체: `<table><tbody><tr>` → React/Next.js + CSS Modules div
    현 방식: 페이지 로드 시 호출되는 `partner-api.kream.co.kr/api/v1/market/asks?status=shipped`
            응답을 가로채고, 페이징은 컨텍스트 request로 직접 호출.
    """
    print("📦 발송관리 내역 수집 시작 (API 인터셉트)...")
    shipments = []
    api_marker = "/api/v1/market/asks"
    captured = {"body": None}

    async def _on_response(resp):
        if (
            captured["body"] is None
            and api_marker in resp.url
            and "status=shipped" in resp.url
        ):
            try:
                ct = (resp.headers.get("content-type") or "").lower()
                if "json" in ct:
                    captured["body"] = await resp.json()
                    print(f"  → API 응답 캡처: cursor 등 페이징 파라미터 포함 URL")
            except Exception as ex:
                print(f"  ⚠ API 응답 파싱 실패: {ex}")

    page.on("response", _on_response)

    try:
        # 페이지 진입 — per_page=100로 한 번에 많이 가져오기
        await page.goto(
            f"{PARTNER_URL}/business/asks/shipment?status=shipped&per_page=100",
    ...
    # 폴백 1: API 자동 캡처 실패 시 context.request로 직접 호출
    if not isinstance(body, dict):
        print("  ⚠ API 자동 캡처 실패 — context.request로 직접 호출 폴백")
        try:
            api_url = (
                "https://partner-api.kream.co.kr/api/v1/market/asks"
                "?status=shipped&per_page=100&cursor=1"
            )
            resp = await page.context.request.get(api_url)
            if resp.ok:
                body = await resp.json()
                print(f"  → 직접 호출 성공: total={body.get('total')}")
            else:
                print(f"  ❌ 직접 호출 실패: HTTP {resp.status}")
                return shipments
        except Exception as e:
            print(f"  ❌ 직접 호출 예외: {e}")
            return shipments
    ...
    # 페이지 순회 (max_pages 제한, KREAM API의 next_page cursor 사용)
    for page_num in range(1, max_pages + 1):
        items = body.get("items", []) or []
        for item in items:
            sh = _convert_api_item_to_shipment(item)
            if sh:
                shipments.append(sh)
        total = body.get("total", "?")
        print(f"  📄 page={page_num} 수집 {len(items)}건 (누적 {len(shipments)} / total {total})")
        ...
        try:
            api_url = (
                "https://partner-api.kream.co.kr/api/v1/market/asks"
                f"?status=shipped&per_page=100&cursor={next_cursor}"
            )
            resp = await page.context.request.get(api_url)
            if not resp.ok:
                print(f"  ⚠ next page (cursor={next_cursor}) HTTP {resp.status}")
                break
            body = await resp.json()
        except Exception as e:
            print(f"  ⚠ next page 호출 예외: {e}")
            break
...
def _convert_api_item_to_shipment(item):
    """KREAM partner-api `/api/v1/market/asks` item → sales_history 호환 dict."""
    ...
    try:
        ...
        try:
            sale_price = int(item.get("price") or 0)
        except (TypeError, ValueError):
            sale_price = 0

        rid = release.get("id")
        return {
            "order_id": item.get("oid", "") or "",
            "product_id": str(rid) if rid else "",
            "model": release.get("style_code", "") or "",
            "product_info": release.get("translated_name") or release.get("name") or "",
            "size": item.get("product_option_name_display", "") or "",
            "sale_price": sale_price,
            "trade_date": _iso_date(transaction.get("date_created", "")),
            "ship_date": _iso_date(item.get("date_paid", "")),
            "ship_status": item.get("status_display", "") or "",
        }
    except Exception as e:
        print(f"  ⚠ 항목 변환 실패: {e}")
        return None
