# KREAM 판매자센터 자동화 프로젝트 — 인수인계서 v6 (2026-04-24)
...
## 3. kream_server.py — API 엔드포인트 전체 목록 (108개)
...
| POST | `/api/auto-rebid/toggle` | 자동 재입찰 ON/OFF | `{enabled: bool}` |
| POST | `/api/auto-rebid/run-once` | 수동 1회 실행 (최근 1시간 판매 대상) | |
| GET | `/api/auto-rebid/history` | 재입찰 이력 | `?limit=50&filter=all&from_date=&to_date=` |
...
## 7. DB 테이블 구조 (price_history.db — 18개 테이블)
...
```sql
id, original_order_id, model, size, sold_price, new_bid_price,
expected_profit, action, skip_reason, new_order_id, executed_at
...
-- new_order_id: place_bid() Boolean 반환 한계로 NULL 허용
...
| **자동 재입찰** [v6 NEW] | 판매 수집 직후 | 새 판매 감지 시 자동 재입찰 (6중 안전장치) |
...
## 10.5 자동 재입찰 시스템 (v6 NEW — 11~21장 번호 유지 위해 10.5 사용)

### 개요
판매 수집 스케줄러에서 새 체결건 감지 시, 같은 품번+사이즈에 자동으로 새 입찰 등록.
주데이님이 매번 수동으로 재입찰할 필요 제거 → 판매 기회 손실 방지.

### 6중 안전장치
| # | 조건 | 설명 |
|---|------|------|
| 1 | 원가 체크 | bid_cost에 원가 데이터 있어야 함 |
| 2 | 루프 가드 | 같은 모델+사이즈 24시간 내 5회 초과 시 차단 |
| 3 | 가격 급변 | 재입찰가가 판매가 ±10% 벗어나면 차단 (시장 이상 감지) |
| 4 | 하루 한도 | 기본 20건/일 (설정 변경 가능) |
| 5 | 블랙리스트 | 특정 모델 제외 가능 (settings의 auto_rebid_blacklist) |
| 6 | 마진 하한 | expected_profit >= 4,000원 |

### 실행 플로우
1. 판매 수집 스케줄러(`_run_sales_sync`)가 새 판매 감지
2. `auto_rebid_enabled=true` 확인
3. `auto_rebid_after_sale(new_items)` 호출
4. 모델별 그룹핑 → KREAM 가격 1회 수집 (부하 최소화)
5. 사이즈별 6개 조건 체크
6. 통과 건 → `_execute_rebid()` 실행 (새 브라우저 세션 기동 → place_bid)
7. 모든 결과 `auto_rebid_log`에 기록

### 기본 설정 (settings.json)
```json
{
  "auto_rebid_enabled": false,      // 기본 OFF (사용자 명시적 ON 필요)
...
### 자기 입찰 제외 로직
- 재입찰가 계산 시 `my_bids_local.json` 조회
- 내 다른 입찰이 최저가면 → 자기 언더컷 방지 위해 skip (skipped_margin_low)
- userMemories 핵심 원칙 준수

### 알려진 제약
- `place_bid()`가 Boolean만 반환 → `auto_rebid_log.new_order_id`는 NULL
- 실제 새 입찰 주문번호 추적은 추후 `my-bids/sync`로 역매칭 가능
- sales_history가 아직 0건이라 실제 동작 검증은 첫 판매 발생 후
...
## 13. v5→v6 변경사항 (2026-04-24)

### Step 4: 자동 재입찰 시스템
- `auto_rebid_log` 테이블 신설 (인덱스 2개: idx_rebid_executed, idx_rebid_model_size)
- `auto_rebid_after_sale()` 엔진 구현 (판매 수집 스케줄러 `_run_sales_sync` 연동)
- 6중 안전장치: 원가 / 루프 / 가격급변 / 하루한도 / 블랙리스트 / 마진
- 자기 입찰 제외 로직 (my_bids_local 참조)
- 모델별 KREAM 가격 그룹 수집으로 부하 감소
- API 4개: `/api/auto-rebid/{status, toggle, run-once, history}`
- UI: tab_sales 상단 패널 + 이력 테이블, tab_settings 블록
- `place_bid()` Boolean 반환 한계로 `new_order_id`는 NULL 허용
- 기본 OFF (`auto_rebid_enabled: false`)
- 커밋 해시: `1c67013`

### 부가 작업
- `tab_adjust.html` Step 3 UI 누락분 보완 (자동 실행 패널 추가)
- 테스트 시나리오 9개 전원 PASS (문법 / 재시작 / 헬스 / API 4개 / DB / 설정 / 기본OFF / UI / 블랙리스트 로직)

### 작업 방식 학습
- 작업지시서 v2 (1,265줄) 사용 — 선행 탐색 9개 영역 의무화
- Claude Code가 코드베이스 먼저 탐색 → 탐색 결과 기반 구현
- 지시서의 함수명/변수명은 설계도일 뿐, 실제 코드 우선
- 롤백 계획 + 긴급 OFF 명령 포함

---

## 14. v4→v5 변경사항 (2026-04-18 ~ 2026-04-24)
...
| auto_rebid_log | 0 | [v6 NEW] 자동 재입찰 이력 (판매 없어서 0건) |
| sales_history | 0 | 판매 내역 (실제 체결 없음) |

---

## 18. 다음 작업 예정

- **Step 5:** 입찰 정리 도구 (다음 작업)
- **Step 6:** 인기 정의 시스템
- **Step 7:** 데이터 분석 / 상품 발굴 고도화

**대기 중 과제:**
- 실제 판매 발생 시 auto_rebid_log 모니터링 → 6중 안전장치 실전 검증
- `place_bid()` orderId 반환 리팩토링 (장기 과제)
- 헬스체크 `status: critical` 원인 개선 (last_sale_age_hours NULL일 때 예외 처리)
