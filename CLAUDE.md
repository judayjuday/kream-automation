...
## 절대 규칙 #7: 인보이스 단가는 시스템 데이터로 사용 금지

**배경:**
인보이스(INVOICE) PDF에 기재된 ITEM별 PRICE/AMOUNT는 통관/세무 신고용
명목 단가이며, 실제 매입 원가와 다를 수 있습니다.

**금지 사항:**
- 인보이스의 단가를 자동 파싱해서 model_price_book에 시드 금지
- 인보이스 단가로 마진 계산 금지
- 인보이스의 ITEM 단가를 bid_cost 등 시스템 의사결정 데이터로 사용 금지

**허용 사항:**
- 인보이스 PDF/이미지를 영수증으로 첨부 (receipt_type='invoice')
- 인보이스 번호를 송금 메모/설명에 기록
- 인보이스 발행 일시 및 협력사 거래 이력 추적용 보관

**데이터 신뢰도 우선순위 (재확인):**
1. bid_cost (실제 입찰 시점 원가) ← 진실값
2. model_price_book (사장님 직접 입력) ← 폴백
